...
$endpos$集合大小就相当于子串出现次数，求出这个就能~~乱搞~~干很多事情了。
...
对第一个串造$SAM$，每个节点维护**已匹配过的串走到该节点的最小长度$mi$**和**当前串走到该节点最大长度$ma$**。让每个串往上面跑，每次走到某个节点都用当前长度对$ma$取$\max$，跑完一个串逐步更新每个节点，$mi_i$对$ma_i$取$\min$，$\min\{ma_i,len[fa[i]]\}$对$ma_{fa[i]}$取$\max$。答案为$\max\{mi_i\}$。

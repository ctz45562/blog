...
[板子](https://vijos.org/d/Bashu_OIers/p/5a79a3e1d3d8a103be7e2b81)
...
<span class="spoiler">其实这份代码6,7,8会RE，但是我懒得调了。</span>

``` cpp
vector<int>u[maxn],v[maxn];
int son[maxn],md[maxn],fa[maxn][22],lg[maxn],deep[maxn],h[maxn],top[maxn],num,cnt;
struct edge{int pre,to;}e[maxn<<1];
inline void add(int from,int to){e[++num]=(edge){h[from],to},h[from]=num;}
void dfs1(int node=1){
	md[node]=1;
	for(register int j=1;j<=lg[deep[node]];++j)
		fa[node][j]=fa[fa[node][j-1]][j-1];
	for(register int i=h[node],x;i;i=e[i].pre){
		x=e[i].to;
		if(md[x])continue;
		deep[x]=deep[node]+1,fa[x][0]=node,dfs1(x);
		if(md[x]>md[son[node]])md[node]=md[x]+1,son[node]=x;
	}
}
void dfs2(int node=1){
	if(top[node]==node){
		for(register int i=0,x=node;i<=md[node]&&x;++i,x=fa[x][0])u[node].push_back(x);
		for(register int x=son[node];x;x=son[x])v[node].push_back(x);
	}
	if(!son[node])return;
	top[son[node]]=top[node],dfs2(son[node]);
	for(register int i=h[node],x;i;i=e[i].pre){
		x=e[i].to;
		if(x!=fa[node][0]&&x!=son[node])top[x]=x,dfs2(x);
	}
}
inline int query(int x,int k){
	if(k>deep[x])return 0;
	if(!k)return x;
	x=fa[x][lg[k]],k^=1<<lg[k];
	if(k>=deep[x]-deep[top[x]])return u[top[x]][k-deep[x]+deep[top[x]]];
	return v[top[x]][deep[x]-deep[top[x]]-k-1];
}
int main(){
	top[1]=1;
	int n=read(),ans=0;
	for(register int i=2,x,y;i<=n;++i)lg[i]=lg[i>>1]+1,x=read(),y=read(),add(x,y),add(y,x);
	dfs1(),dfs2();
	int m=read();
	while(m--){
